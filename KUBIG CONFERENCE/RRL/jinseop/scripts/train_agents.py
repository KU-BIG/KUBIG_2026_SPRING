...
from oai_agents.common.tags import TeamType, AdversaryPlayConfig, KeyCheckpoints, TeammatesCollection
...
from oai_agents.common.teammates_collection import update_TC_w_ADV_teammates
...
from scripts.utils.common import load_agents, generate_name
...
def SPN_1ADV_PWADV_FINAL_ADVONLY(args) -> None:
    '''
    Standalone re-run of round2's PWADV_final from an existing SPN_1ADV run, with two changes:
      1. Reuses the already-trained population and ADV_2 (round-2 adversary) from disk instead
         of redoing Stage 1 (population SP) and rounds 0/1 (ADV_0/PWADV_0, ADV_1/PWADV_1).
      2. Every episode is [ADV_2 + 1 self-copy] (TeamType.SELF_PLAY_ADVERSARY only) -- there are
         no pure self-play (all-primary) episodes, unlike the original PWADV_final which mixed
         SELF_PLAY and SELF_PLAY_ADVERSARY.
    Trains for 60M timesteps (4x n_x_sp_total_training_timesteps), matching the original
    PWADV_final's schedule for the last attack round.
    '''
    unseen_teammates_len = 1
    attack_round = 2

    adv_name = generate_name(
        args,
        prefix='ADV',
        seed=args.ADV_seed,
    ...
    adv_agents = load_agents(args, name=adv_name, tag=KeyCheckpoints.MOST_RECENT_TRAINED_MODEL)
    assert adv_agents, f'Could not find previously trained adversary agent: {adv_name}'
    adversary_agent = adv_agents[0]

    curriculum = Curriculum(train_types=[TeamType.SELF_PLAY_ADVERSARY], is_random=True)
    ...
    name = generate_name(
        ...
        curriculum=curriculum,
        suffix=args.primary_learner_type + f'_attack{attack_round}_advonly',
    )
    ...
    teammates_collection = {
        TeammatesCollection.TRAIN: {layout: {} for layout in args.layout_names},
        TeammatesCollection.EVAL: {layout: {} for layout in args.layout_names},
    }
    teammates_collection = update_TC_w_ADV_teammates(
        args=args,
        teammates_collection=teammates_collection,
        adversaries=[adversary_agent],
        primary_agent=random_init_agent,
        adversary_play_config=AdversaryPlayConfig.SAP,
    )
    ...
    return n_x_sp_types_trainer.get_agents()[0]
...
if __name__ == '__main__':
    ...
    elif args.algo_name == 'SPN_1ADV_PWADV_FINAL_ADVONLY':
        SPN_1ADV_PWADV_FINAL_ADVONLY(args=args)
