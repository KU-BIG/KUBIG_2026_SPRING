...
TARGET_VAL_ACC = 0.97
MAX_ATTEMPTS = 4
RUN_ROOT = Path('/workspace/team_type_classifier_runs')
...
def diagnose_and_adjust(diagnostics, config, dataset_n):
    ...
    if gap > 0.06:
        ...
        reasons.append('Train accuracy itself is low => underfitting. Increasing model capacity '
                        '(hidden/embed dim, bidirectional) and training longer.')
...
def main():
    ...
    for attempt in range(1, MAX_ATTEMPTS + 1):
        ...
        # train on however many episodes have been generated so far (>= dataset_n)
