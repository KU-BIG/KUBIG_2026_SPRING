...
SMILES_CSV = DATASET_ROOT / "ogbg_molhiv" / "mapping" / "mol.csv.gz"
...
# TDA defaults
PI_RESOLUTION = 5
PI_SIGMA = 0.5  # ~half a pixel; 0.05 was far too narrow and produced all-zero images
...
# Graph-topology (hop-distance) Rips filtrations
...
# Partial-charge sublevel-set filtration (lens D). Gasteiger charges are small
# floats (~[-0.6, 0.6]), so the persistence-image sigma must be scaled to that
# range -- reusing the integer-scale PI_SIGMA=0.5 would wash the image flat
# (the inverse of the all-zero bug fixed earlier).
CHARGE_TDA_CACHE = CACHE_ROOT / "charge_tda.pt"          # D: Gasteiger charge filtration
CHARGE_TDA_DIM = PI_RESOLUTION * PI_RESOLUTION * 2       # H0 + H1 persistence images
CHARGE_BIRTH_RANGE = (-0.6, 0.6)                         # birth = atom charge
CHARGE_PERS_RANGE = (0.0, 1.2)                           # persistence = charge span
CHARGE_PI_SIGMA = 0.12                                   # ~half a pixel at charge scale
